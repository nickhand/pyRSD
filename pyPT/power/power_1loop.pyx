...
cpdef Pdd_1loop(k_hMpc, z, kmin=KMIN, kmax=KMAX, 
    ...
    Plin = linear_growth.Pk_full(klin, 0., params=cosmo)
    ...
    D          = linear_growth.growth_function(z, normed=True, params=cosmo)
    f          = linear_growth.growth_rate(z, params=cosmo)
    conformalH = cosmo_tools.H(z,  params=cosmo)/(1.+z)
     
    Plin = linear_growth.Pk_full(k, 0., params=cosmo)
    P11 = Plin
    P22 = 2*I00
    P13 = 3*k**2*J00*Plin
