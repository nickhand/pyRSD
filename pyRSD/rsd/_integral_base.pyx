...
"""
 integral_base.pyx
 pyRSD: Cython base module to compute the necessary perturbation integrals
...
cdef double inner2D(double x,  void * params) nogil:
    ...
    kern = _kernels.kernel(p.kernel_name, q/p.k, x)
...
cdef double integrand1D(double lnq,  void * params) nogil:
    ...
    cdef fparams * p = (<fparams *> params)
    q = exp(lnq)
    kern = _kernels.kernel(p.kernel_name, q/p.k, 0.)
